...
struct Student 
{
  ...
  {
    return !(strcmp(name, s.name)) && score == s.score;
  }
...
private:
  ...
  StudentList &_remove(const char *n) 
  {
    for (size_t i = 0; i < _size; ++i) 
    {
      if (!strcmp(_stud[i].name, n)) 
      {
        for (size_t x = i; x < _size - 1; ++x)
          _stud[x] = _stud[x + 1];
        Student *tmp =
            (Student *)realloc(_stud, (_size - 1) * sizeof(Student));
        if (tmp == NULL && _size > 1)
          exit(EXIT_FAILURE);
        _size--;
        _stud = tmp;
...
public:
  ...
  StudentList(initializer_list<Student> list)
      : _stud{new Student[list.size()]}, _size{list.size()} 
  {
    for (size_t i = 0; const auto &elem : list) 
    {
      _stud[i] = elem;
      i++;
    }
...
public:
  StudentList &operator=(const StudentList &other) 
  {
    if (this == &other)
      return *this;
    _size = other._size;
    _stud = (Student *)realloc(_stud, _size * sizeof(Student));
    for (size_t i = 0; i != _size; ++i)
...
public:
  ...
  StudentList &remove(const char *n) 
  { 
    return _remove(n); 
  }
  StudentList &remove(Student &s) 
  { 
    return _remove(s.name); 
  }

  StudentList &set_score(const char *n, int ns) 
  { 
    return _set_score(n, ns); 
  }
